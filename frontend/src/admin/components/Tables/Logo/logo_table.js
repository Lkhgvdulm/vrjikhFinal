...
const Logo_table = () => {
	...
	async function createSlider(event) {
		...
		const { data } = await axios.post(`${url}/logo_create`, formdata);
		if (data.success) {
	...
	const deleteSlider = async (itemId) => {
		const { data } = await axios.delete(`${url}/logo_delete/${itemId}`);
		if (data.success) {
			setRefreshKey((old) => old + 1);
			Swal.fire({
		...
		<div>
			...
			<div className="flex flex-col">
				<div className="overflow-x-auto sm:-mx-6 lg:-mx-8">
					<div cl assName="py-2 inline-block min-w-full sm:px-6 lg:px-8">
			...
			<Modal opened={opened} onClose={() => setOpened(false)} title="logo">
