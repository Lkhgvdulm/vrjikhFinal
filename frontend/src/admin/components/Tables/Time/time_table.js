...
const Time_table =() => {
	...
	useEffect(() => {
		const getSlider = async () => {
			const { data } = await axios.get(`${url}/time`);
			setTime(data.data);
		};
		getSlider();
	}, [refreshKey]);
	async function createSlider(event) {
		...
		const { data } = await axios.post(`${url}/time_create/`, formdata);
		if(data.success) {
	...
	const deleteSlider = async (itemId) => {
		const { data } = await axios.delete(`${url}/time_delete/${itemId}`);
		if(data.success) {
			setRefreshKey((old) => old +1);
			Swal.fire({
