...
const Goals_table = () => {
	...
	const [goals, setGloals] = useState([]);
	...
	async function createSlider(event) {
		...
		const { data } = await axios.post(`${url}/goals_create`, formdata);
		if (data.success) {
	...
	const deleteGoals = async (itemId) => {
		const { data } = await axios.delete(`${url}/goals_delete/${itemId}`);
		if (data.success) {
			setRefreshKey((old) => old + 1);
			Swal.fire({
