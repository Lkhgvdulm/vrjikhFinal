...
const Hayag_table =() => {
	...
	useEffect(() => {
		const getSlider = async () => {
			const { data } = await axios.get(`${url}/hayag`);
			setSlider(data.data);
		};
	}, [refreshKey]);
	...
	const { data } = await axios.post(`${url}/hayag_create`, formdata);
	if(data.success) {
	...
	const deleteSlider = async (itemId) => {
		const { data } = await axios.delete(`${url}/hayag_delete/${itemId}`);
		if(data.success) {
			setRefreshKey((old) => old +1);
			Swal.fire({
							...
							<table className="min-w-full">
								...
								<tbody> 
									...
									<tr className="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
										...
										<td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
											<img className="w-[100px] h-[80px]" src="" />
										</td>
										<td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
